...
public func showWhisperSimultaneously(whisper message: Message, to: UINavigationController, action: WhisperAction = .show) {
...
public func hideWhisperSimultaneously(whisperFrom from: UINavigationController, after: TimeInterval = 0) {
    WhisperFactory.init().silentWhisper(from, after: after)
}
